...
use witx_bindgen_wasmtime::{anyhow, wasmtime};
...
#[derive(Default)]
pub struct GlassRuntimeData {}
pub struct GlassRuntime<T> {
    get_state: Box<dyn Fn(&mut T) -> &mut GlassRuntimeData + Send + Sync>,
...
impl<T> GlassRuntime<T> {
    ...
    pub fn add_to_linker(
        ...
        get_state: impl Fn(&mut T) -> &mut GlassRuntimeData + Send + Sync + Copy + 'static,
    ) -> anyhow::Result<()> {
        Ok(())
    }
    ...
    pub fn new(
        ...
        let handler = instance
            .get_typed_func::<(i32, i32, i32, i32, i32, i32, i32, i32, i32, i32, i32), (i32,), _>(
                &mut store, "handler",
            )?;
        let memory = instance
    ...
    pub fn handler(
        ...
        for (i, e) in vec3.into_iter().enumerate() {
            let base = result3 + (i as i32) * 8;
            {
                ...
                memory.data_mut(&mut caller).store(
                    base + 4,
                    witx_bindgen_wasmtime::rt::as_i32(vec2.len() as i32),
                )?;
                memory
                    .data_mut(&mut caller)
                    .store(base + 0, witx_bindgen_wasmtime::rt::as_i32(ptr2))?;
            }
        }
            ...
            Some(e) => {
                ...
                for (i, e) in vec5.into_iter().enumerate() {
                    let base = result5 + (i as i32) * 8;
                    {
                        ...
                        memory
                            .data_mut(&mut caller)
                            .store_many(ptr4, vec4.as_ref())?;
                        memory.data_mut(&mut caller).store(
                            base + 4,
                            witx_bindgen_wasmtime::rt::as_i32(vec4.len() as i32),
                        )?;
                        memory
                            .data_mut(&mut caller)
                            .store(base + 0, witx_bindgen_wasmtime::rt::as_i32(ptr4))?;
                    }
                }
                (1i32, result5, len5)
            }
        ...
        let (result8_0, result8_1, result8_2) = match t0_4 {
            ...
            Some(e) => {
                ...
                memory
                    .data_mut(&mut caller)
                    .store_many(ptr7, vec7.as_ref())?;
                (1i32, ptr7, vec7.len() as i32)
            }
        };
        let (result9_0,) = self.handler.call(
            &mut caller,
            (
                t0_0 as i32,
                ptr1,
        ...
        Ok((
            u16::try_from(load10).map_err(bad_int)?,
            match load11 {
                0 => None,
                1 => Some({
                    let len20 = load13;
